<template>
  <header>
    <div class="topbar">
      <div class="container">
        <div class="topbar__text">
          7h-20h du Lundi au Samedi.<br/>
          Créez votre société en ligne ou par téléphone en 5 minutes !
        </div>
        <div class="topbar__phone">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0)">
              <path
                  d="M4.07545 13.2947C6.04809 15.6527 8.42273 17.5093 11.133 18.823C12.165 19.3121 13.545 19.8923 15.0825 19.9917C15.1778 19.9959 15.269 20 15.3643 20C16.3962 20 17.2251 19.6436 17.9006 18.9101C17.9047 18.9059 17.913 18.8976 17.9171 18.8893C18.1575 18.5993 18.431 18.3382 18.717 18.0605C18.9117 17.874 19.1107 17.6792 19.3013 17.4803C20.184 16.5603 20.184 15.3916 19.293 14.5006L16.8023 12.0099C16.3796 11.5707 15.874 11.3386 15.3436 11.3386C14.8131 11.3386 14.3034 11.5707 13.8682 12.0058L12.3846 13.4894C12.2478 13.4107 12.1069 13.3402 11.9743 13.2739C11.8086 13.191 11.6552 13.1123 11.5185 13.0253C10.1674 12.1674 8.94076 11.0485 7.76794 9.61044C7.17532 8.86034 6.77748 8.23042 6.49981 7.58807C6.88937 7.23581 7.25406 6.86697 7.60632 6.50642C7.73065 6.37795 7.85912 6.24948 7.98759 6.12101C8.43516 5.67344 8.67553 5.15541 8.67553 4.62909C8.67553 4.10278 8.43931 3.58475 7.98759 3.13717L6.75261 1.9022C6.60756 1.75715 6.4708 1.61625 6.3299 1.4712C6.05638 1.18939 5.77043 0.899296 5.48862 0.63821C5.06177 0.219644 4.56032 0 4.02986 0C3.50354 0 2.99795 0.219644 2.55452 0.642354L1.00458 2.19229C0.440966 2.75591 0.121861 3.4397 0.0555538 4.23125C-0.0231863 5.22172 0.159159 6.27435 0.6316 7.54662C1.35684 9.51513 2.45091 11.3427 4.07545 13.2947ZM1.06674 4.31828C1.11647 3.76709 1.32783 3.30709 1.72567 2.90924L3.26732 1.36759C3.50769 1.13552 3.77292 1.01533 4.02986 1.01533C4.28266 1.01533 4.5396 1.13552 4.77582 1.37588C5.05348 1.63282 5.31457 1.9022 5.59637 2.18815C5.73728 2.3332 5.88233 2.47824 6.02737 2.62743L7.26235 3.86241C7.51929 4.11935 7.65191 4.38044 7.65191 4.63738C7.65191 4.89432 7.51929 5.15541 7.26235 5.41235C7.13388 5.54082 7.00541 5.67344 6.87694 5.80191C6.49153 6.19146 6.13098 6.5603 5.73313 6.91256C5.72485 6.92085 5.7207 6.92499 5.71241 6.93328C5.36844 7.27725 5.42232 7.60464 5.5052 7.85329C5.50935 7.86573 5.51349 7.87402 5.51763 7.88645C5.83674 8.65313 6.28017 9.38251 6.97225 10.2528C8.21552 11.7862 9.52509 12.9755 10.9673 13.8914C11.1455 14.0075 11.3361 14.0986 11.5143 14.1898C11.6801 14.2727 11.8334 14.3514 11.9702 14.4385C11.9868 14.4467 11.9992 14.455 12.0158 14.4633C12.1525 14.5338 12.2851 14.5669 12.4178 14.5669C12.7493 14.5669 12.9648 14.3556 13.0352 14.2851L14.5852 12.7352C14.8255 12.4948 15.0866 12.3663 15.3436 12.3663C15.6585 12.3663 15.9155 12.5611 16.0771 12.7352L18.5761 15.23C19.0734 15.7273 19.0692 16.2661 18.5636 16.7924C18.3896 16.9789 18.2072 17.1571 18.0125 17.3436C17.7224 17.6254 17.4198 17.9155 17.1463 18.2429C16.6697 18.7567 16.102 18.9971 15.3684 18.9971C15.298 18.9971 15.2234 18.993 15.1529 18.9888C13.7936 18.9018 12.5297 18.3713 11.5806 17.9196C9.00292 16.6722 6.74018 14.9026 4.86285 12.6564C3.31705 10.7957 2.27685 9.06341 1.58891 7.2068C1.16206 6.06714 1.00044 5.15126 1.06674 4.31828Z"
                  fill="#1C3041"/>
            </g>
            <defs>
              <clipPath id="clip0">
                <rect width="20" height="20" fill="white"/>
              </clipPath>
            </defs>
          </svg>
          <span>06 06 06 06 06</span>
        </div>
      </div>
    </div>
    <div class="navbar">
      <div class="container">
        <router-link :to="{name: 'home'}" class="navbar__brand">
          <img class="navbar__logo" :src="require('@img/logo-red-test.png')" alt="Logo"/>
          <div :class="{'navbar__toggler': true, 'active': isNavOpen}" @click="isNavOpen = !isNavOpen">
            <div></div>
            <div></div>
            <div></div>
          </div>
        </router-link>
        <nav :class="{'navbar__nav': true, 'active': isNavOpen}">
          <router-link :to="{name: 'home'}" class="navbar__link">Accueil</router-link>
          <div class="navbar__dropdown">
            <span>Services</span>
            <svg width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                  d="M3.64645 4.35355C3.84171 4.54882 4.15829 4.54882 4.35355 4.35355L7.53553 1.17157C7.7308 0.976311 7.7308 0.659728 7.53553 0.464466C7.34027 0.269204 7.02369 0.269204 6.82843 0.464466L4 3.29289L1.17157 0.464466C0.976311 0.269204 0.659728 0.269204 0.464466 0.464466C0.269204 0.659728 0.269204 0.976311 0.464466 1.17157L3.64645 4.35355ZM3.5 3V4H4.5V3H3.5Z"/>
            </svg>
            <div class="navbar__dropdown-content">
              <router-link :to="{name: 'pricing-create-company'}">Création de société</router-link>
              <router-link :to="{name: 'pricing-edit-company'}">Modification de société</router-link>
              <router-link :to="{name: 'pricing-transfer-company'}">Cession de société</router-link>
            </div>
          </div>
          <router-link :to="{name: 'home'}" class="navbar__link">Contact</router-link>
          <router-link :to="{name: 'home', hash: '#tarifs'}" class=" button button-primary">Nos Tarifs</router-link>
        </nav>
      </div>
    </div>
  </header>

</template>

<script>
export default {
  name: "Navbar",
  data() {
    return {
      isNavOpen: false,
    }
  },
  methods: {
    toggleNav() {

    }
  }
}
</script>

<style scoped>

</style>
